...
An alternative implementation could check out the repo and use `git fetch` + `git log` to compare branches. The GitHub Compare API likely performs a similar comparison on GitHub's servers. The practical difference is **where** the work happens:
